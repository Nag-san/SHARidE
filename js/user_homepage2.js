...
document.getElementById('from').addEventListener('click', function select() {
  ...
  //To go back
document.getElementById('Previous1').addEventListener('click', function Previous1() {
    window.parent.postMessage('user_homepage.html', '*');
  });

  console.log("user_homepage2")
  
document.getElementById("ShareRide").addEventListener('click', async function Next1() {
   ...
   if(choice=='sharee')
   window.parent.postMessage("sharee_page.html",'*');
   else
   window.parent.postMessage("sharer_page.html",'*');
  });